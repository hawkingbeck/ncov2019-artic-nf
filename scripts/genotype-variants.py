...
class RecipeDirectedGraph:
  def __init__(self, recipes: List[dict]):
    ...
    for recipe in recipes:
      ...
      if recipe_name not in self.graph:
        self.graph[recipe_name] = Node(
          name=recipe_name, child_names=[], parent_name=None)
      ...
      if "requires" in recipe:
        ...
        if parent_recipe_name not in self.graph:
          self.graph[parent_recipe_name] = Node(
            name=parent_recipe_name, child_names=[], parent_name=None)
  ...
  def is_single_branch(self):
    ...
    if len(self.graph.keys()) < 2:
      raise ValueError(
        "Can only calculate leaf in graph with at least 2 nodes")
